...
class ArcWeldingData(BaseData):
    """
    A class representing the Arc Welding dataset.
    """

    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        """
        Initializes an instance of the ArcWeldingData class.

        Args:
            root_dir (str): The root directory containing all individual .csv files.
            file_list (list, optional): A list of file paths within `root_dir` to consider. If not provided, the entire `root_dir` contents will be used.
            pattern (str, optional): A regex string to select a subset of files. Defaults to None.
            n_proc (int, optional): The number of processes to use. Defaults to 1.
            limit_size (int or float, optional): The maximum number of samples to load. If an integer, it represents the exact number of samples. If a float between 0 and 1, it represents the proportion of samples to load. Defaults to None.
            config (dict, optional): Additional configuration options. Defaults to None.
        """
        self.set_num_processes(n_proc=n_proc)
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        Returns:
            all_df (pd.DataFrame): A single (possibly concatenated) dataframe with all data corresponding to specified files.
            labels_df (pd.DataFrame): A dataframe containing the labels for the data.
        """
        # each file name corresponds to another date. Also tools (A, B) and others.
    ...
    def load_single(self, filepath):
        ...
        labels = pd.Series(df['labels'], dtype="category")
        self.class_names = labels.cat.categories
    ...
    def select_columns(df):
        ...
        Returns:
            df (pd.DataFrame): The dataframe with selected columns.
        """
        df = df[["ID","current", "voltage", "labels"]]
        return df
